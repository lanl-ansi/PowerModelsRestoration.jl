""
function constraint_model_voltage_damage(pm::_PMs.AbstractACPModel, n::Int)
end
...
""
function constraint_bus_voltage_violation_damage(pm::_PMs.AbstractACPModel, n::Int, i::Int, vm_min, vm_max)
    vm = _PMs.var(pm, n, :vm, i)
    vm_vio = _PMs.var(pm, n, :vm_vio, i)
...
""
function constraint_bus_voltage_violation(pm::_PMs.AbstractACPModel, n::Int, i::Int, vm_min, vm_max)
    vm = _PMs.var(pm, n, :vm, i)
    vm_vio = _PMs.var(pm, n, :vm_vio, i)
...
function constraint_bus_voltage_on_off(pm::_PMs.AbstractACPModel; nw::Int=pm.cnw, kwargs...)
...
""
function constraint_power_balance_shed(pm::_PMs.AbstractACPModel, n::Int, i::Int, bus_arcs, bus_arcs_dc, bus_arcs_sw, bus_gens, bus_storage, bus_pd, bus_qd, bus_gs, bus_bs)
    vm   = _PMs.var(pm, n, :vm, i)
    ...
    qs   = get(_PMs.var(pm, n),   :qs, Dict()); _PMs._check_var_keys(qs, bus_storage, "reactive power", "storage")
    psw  = get(_PMs.var(pm, n),  :psw, Dict()); _PMs._check_var_keys(psw, bus_arcs_sw, "active power", "switch")
    qsw  = get(_PMs.var(pm, n),  :qsw, Dict()); _PMs._check_var_keys(qsw, bus_arcs_sw, "reactive power", "switch")
    p_dc = get(_PMs.var(pm, n), :p_dc, Dict()); _PMs._check_var_keys(p_dc, bus_arcs_dc, "active power", "dcline")
    q_dc = get(_PMs.var(pm, n), :q_dc, Dict()); _PMs._check_var_keys(q_dc, bus_arcs_dc, "reactive power", "dcline")
    ...
    _PMs.con(pm, n, :kcl_p)[i] = JuMP.@NLconstraint(pm.model,
    ...
    _PMs.con(pm, n, :kcl_q)[i] = JuMP.@NLconstraint(pm.model,
